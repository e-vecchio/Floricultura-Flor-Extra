Nomes dos alunos do grupo.

Introdução: apresentação da organização escolhida e do objetivo do site.

Desenvolvimento:
Relato de como foi feita a entrevista/visita à organização real (presencial ou remota por Zoom ou Google Meeting, não será aceito somente conversa por Discord/WhatsApp/Telegram ou semelhantes).
Foto que comprove o contato com o entrevistado e a forma de contato utilizada (ex.: foto da visita presencial, print de conversa, e-mail, etc.).
Explicação do processo de desenvolvimento do site em si: decisões de estrutura, conteúdo levantado, desafios técnicos.

Conclusão: reflexão do grupo sobre o resultado e aprendizados da etapa.

https://e-vecchio.github.io/Floricultura-Flor-Extra/
